...
- 📱 **默认全屏敲击模式**：启动时默认开启全屏点击，手指点击屏幕任意空白区域均可正常敲击木鱼。
- ⏱️ **自动敲击木鱼（解放双手）**：
  - 顶栏右侧提供独立的自动敲击调节按钮。
...
- 🧭 **人体工学顶栏布局**：
  - **左侧功能区**：`[🎵 本地BGM]` + `[✨ 动效]` + `[🔊 音效 1/2]`（支持一键快速切换）
  - **右侧控制区**：`[⏱️ 自动敲击]` + `[👁️ 清屏]` + `[⚙️ 设置]`（设置固定位于最右侧）
- 👁️ **沉浸清屏模式**：
  - 点击右上角【清屏】后，仅清除屏幕中心的功德数字与文字，顶栏所有按钮完整保留常驻，随心操控。
- ✨ **真实物理打击动效**：
  - 模拟真实受力下压、挤压形变（拉伸 X，压缩 Y）与微倾斜，配合高阻尼弹簧回弹。
  - 伴随原生马达轻微触觉震动反馈，支持随时一键开启/关闭。
- ⚙️ **调节与统计清零**：右上角调节弹窗支持查看/更换/清除背景音乐、自由调整 BGM 音量、全屏点击开关以及一键清零已累积功德。
...
- **音频引擎**：`SoundPool`（零延迟敲击） + `MediaPlayer`（本地 Uri BGM 循环播放）
